  ...
  return (
    <div>
      <div className="terminal-logo" style={{ marginLeft: "2em" }}>
        <div className="logo terminal-prompt"><a href="https://twitter.com/unblock256" target="_blank" className="no-style">un.Block</a></div>
      </div>
      <div className='container'>
        {
          sortedTags.map(
            tag => <CategoryCard key={tag} tag={tag}></CategoryCard>
          )
        }
        {data.__type.enumValues.map(item => {
          if (!sortedTags.includes(item.name)) {
            return (< CategoryCard key={item.name} tag={item.name}></CategoryCard>)
          }
        })
        }
      </div >
